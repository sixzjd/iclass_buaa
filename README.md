...
### 2026-09-22 — 修复签到恒报 `ERRCODE 100 参数错误!`

- **根因**：`stu_scan_sign.action` 要求提交的毫秒时间戳**不能落在服务端的未来** —— 实测容差不到 100ms（+0ms 接受、+100ms 就被判 `ERRCODE 100 参数错误!`；而过去方向 −60s 仍不报 100）。本机时钟比 iClass 快约 1 秒时，程序自造的 `Date.now()` 就恒定落在服务端的未来，于是每次签到都失败。与“老师没开签到”“参数名写错”无关。
- **v1.0.4 的修法为什么不够**：它用服务器 `Date` 响应头算时间偏移，但该响应头**只有秒级精度**，算出的偏移在 1 秒内是随机的。实测连续 6 次登录，偏移在 −1563ms ~ −755ms 之间跳，其中 **3 次**造出的时间戳落在服务端未来 ⇒ 仍被判参数错误。这正是该问题“有时能签、有时报错”的原因。
- **修复**：签到前先向 iClass 索取它自己下发的时间戳（`GET /app/common/get_timestamp.action`）再提交。同一节课实测对照：本机时间戳 → `ERRCODE 100`；服务端下发时间戳 → 请求被接受。
- **不再谎报成功**：原先只要签到接口返回 `STATUS='0'` 就显示“签到成功”。现在必须复检 `get_stu_course_sched` 的 `signStatus === '1'` 才算成功，否则明确报出 `ERRCODE`/`ERRMSG`。
...
PR 链接： https://github.com/zeroduhyy/iclass_buaa/pull/20

特别感谢本次修复的 PR 提交者：[@Yiki21](https://github.com/Yiki21)
