 ...
 * 签到接口只接受落在 **[服务端时间 −3s, +1s]** 内的毫秒时间戳：
 *   服务端 −3s -> 接受   服务端 −4s -> ERRCODE 101 二维码已失效！
 *   服务端 +1s -> 接受   服务端 +2s -> ERRCODE 100 参数错误!
 * 而这台机器的系统时钟比 iClass 签到服务**快约 4.7 秒**，所以本机 Date.now()
 * 恒定落在窗口外 —— 这才是此前每次签到都报 100 参数错误的真正原因，
 * 与"老师没开签到""参数名写错"都无关。
